...
class ParquetExtractor(IBaseExtractor):
    ...
    def __init__(
        ...
        """
        Initialize Parquet extractor with configuration.

        Args:
            schema: Predefined schema (optional, Parquet files usually have schema embedded)
            options: Additional Spark Parquet reader options
        """
        self.schema = schema
    ...
    def extract(self, file_path: str) -> DataFrame:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                raise ExtractionError(f"Parquet file not found: {file_path}")
            ...
            if df.count() == 0:
                raise ExtractionError(f"Parquet file is empty: {file_path}")
            ...
            raise ExtractionError(
                f"Failed to extract Parquet from {file_path}: {str(e)}"
            )
    ...
    def extract_with_validation(
        ...
        """
        Extract Parquet data with additional validation.
        ...
        if row_count < min_rows:
            raise ExtractionError(
                f"Insufficient rows in Parquet file. Found {row_count}, expected at least {min_rows}"
            )
